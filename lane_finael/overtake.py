...
def main():
    ...
    agent3 = Agent(3, [0,agent1.c_state[1]+5,np.deg2rad(90)],[0,45,np.deg2rad(85)], vg, wg, p_horizon, u_horizon)
    ...
    y_u_lim = 100
    update_y = 0
    while( ( (np.linalg.norm(agent1.c_state-agent1.g_state)>th) or (np.linalg.norm(agent2.c_state-agent2.g_state)>th) \
        (np.linalg.norm(agent3.c_state-agent3.g_state)>th) or (np.linalg.norm(agent4.c_state-agent4.g_state)>th) ) and timeout>0):
    ...
    #         dist1.append(get_dist(agent1.c_state, agent2.c_state))
        ...
        agent4.g_state[1] = agent4.c_state[1] + 50    
        agent4.vl = agent4.v
        agent4.wl = agent4.w
